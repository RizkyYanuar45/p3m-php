...
<head>
    ...
    <title>Kelola SK Rektor Penelitian</title>
</head>
...
<body class="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display">
        ...
        <main class="flex-1 flex flex-col overflow-hidden pt-14 lg:pt-0">
            <div class="flex-1 overflow-y-auto p-4 md:p-8 bg-background-light dark:bg-background-dark custom-scrollbar">
                <header class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 class="text-xl md:text-3xl font-black tracking-tight mb-2">Kelola SK Rektor Penelitian</h2>
                        <p class="text-slate-500">Tambah, edit, atau hapus berkas SK Rektor penelitian dalam sistem.</p>
                    </div>
                    <a href="/sk-rektor-penelitian" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg font-bold text-sm transition-all border border-slate-200 dark:border-slate-700">
                ...
                <section class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 mb-8 shadow-sm">
                    ...
                    <form action="/admin/penelitian/sk-rektor/tambah" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="flex flex-col gap-2">
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Nama File</label>
                            <input name="file_name" required class="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:border-primary focus:ring-primary h-12" placeholder="Contoh: SK Libur Penelitian" type="text" />
                        </div>
                        <div class="flex flex-col gap-2">
    ...
    <div id="modal-edit" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form action="/admin/penelitian/sk-rektor/edit" method="POST" class="p-6 space-y-4">
    ...
    <div id="modal-delete" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden text-center p-8">
            ...
            <form action="/admin/penelitian/sk-rektor/hapus" method="POST" class="flex flex-col gap-3">
