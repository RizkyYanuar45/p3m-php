...
<head>
    ...
    <title>Kelola Informasi Penelitian</title>
</head>
...
<body class="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div class="flex flex-col md:flex-row md:items-center gap-4">
                        <div>
                            <h1 class="text-xl md:text-3xl font-black tracking-tight">Kelola Informasi Penelitian</h1>
                            <p class="text-slate-500 dark:text-slate-400 mt-1">Daftar artikel penelitian yang telah diterbitkan atau sedang dalam tahap draf.</p>
                        </div>
                        <a href="/informasi_penelitian" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg font-bold text-sm transition-all border border-slate-200 dark:border-slate-700">
                                ...
                                <?php if (empty($articles)): ?>
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-slate-500">
                                            ...
                                            <p>Belum ada artikel informasi penelitian yang ditemukan.</p>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                        ...
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <div class="flex items-center justify-end gap-2 text-right">
                                                    <a href="/artikel/<?= htmlspecialchars($article['slug']); ?>" target="_blank" class="p-1.5 text-slate-400 hover:text-primary transition-colors" title="Lihat">
    ...
    <div id="modal-add" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form action="/admin/penelitian/informasi/tambah" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-2 md:col-span-2">
                        <label class="text-sm font-semibold">Judul Artikel</label>
                        <input name="title" required class="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:border-primary focus:ring-primary h-11" type="text" placeholder="Masukkan judul artikel..." />
                    </div>
                    <div class="space-y-2">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <textarea name="content" required class="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:border-primary focus:ring-primary p-3" rows="8" placeholder="Tuliskan konten artikel di sini..."></textarea>
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button type="submit" class="bg-primary hover:bg-orange-600 text-white px-8 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-primary/20 transition-all">Terbitkan Artikel</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal-edit" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form action="/admin/penelitian/informasi/edit" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
    ...
    <div id="modal-delete" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden p-8 text-center animate-in fade-in zoom-in duration-200">
            ...
            <p class="text-slate-500 dark:text-slate-400 text-sm mb-8">Tindakan ini tidak dapat dibatalkan. Seluruh konten artikel ini akan dihapus permanen.</p>
            <form action="/admin/penelitian/informasi/hapus" method="POST" class="flex flex-col gap-3">
    ...
    <script>
        ...
        // Simple Search
        document.querySelector('input[placeholder="Cari judul artikel atau penulis..."]').addEventListener('input', function(e) {
            const term = e.target.value.toLowerCase();
            const rows = document.querySelectorAll('#article-table-body tr:not(:has(td[colspan]))');
