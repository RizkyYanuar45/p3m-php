...
<head>
    ...
    <title>Kelola Dokumen Mandiri - Admin</title>
</head>
...
<body class="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display">
        ...
        <main class="flex-1 flex flex-col overflow-hidden pt-14 lg:pt-0">
            <div class="flex-1 overflow-y-auto p-4 md:p-8 bg-background-light dark:bg-background-dark custom-scrollbar">
                <header class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 class="text-xl md:text-3xl font-black tracking-tight mb-2">Kelola Dokumen Pengabdian Mandiri</h2>
                        <p class="text-slate-500">Manajemen berkas dokumen pengabdian masyarakat mandiri.</p>
                    </div>
                    <a href="/pengabdian-kepada-masyarakat-mandiri" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg font-bold text-sm transition-all border border-slate-200 dark:border-slate-700">
                ...
                <section class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 mb-8 shadow-sm">
                    <div class="flex items-center gap-2 mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
                        ...
                        <h3 class="text-lg font-bold">Tambah Dokumen Baru</h3>
                    </div>
                    <form action="/admin/pengabdian/dokumen-mandiri/tambah" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="flex flex-col gap-2">
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Nama Dokumen</label>
                            <input name="file_name" required class="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:border-primary focus:ring-primary h-12" placeholder="Contoh: Laporan Akhir Mandiri 2023" type="text" />
                        </div>
                        <div class="flex flex-col gap-2">
                        ...
                        <div class="flex flex-col gap-2 md:col-span-2">
                            ...
                            <textarea name="file_description" class="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:border-primary focus:ring-primary" placeholder="Deskripsi singkat..." rows="3"></textarea>
                        </div>
                ...
                <section class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                        ...
                        <div class="relative">
                            ...
                            <input class="pl-9 pr-4 py-2 rounded-full border-slate-200 dark:border-slate-700 dark:bg-slate-800 text-sm focus:border-primary focus:ring-primary" placeholder="Cari..." type="text" />
                        </div>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-slate-800/50">
                                    <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase w-16">No</th>
                                    <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Nama Dokumen</th>
                                    <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Deskripsi</th>
                                    <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800" id="file-table-body">
                                <?php if (empty($files)): ?>
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center text-slate-500">Belum ada dokumen yang ditemukan.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($files as $index => $file): ?>
                                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                            <td class="px-6 py-4 text-sm text-slate-400"><?= ($page - 1) * $limit + $index + 1 ?></td>
                                            <td class="px-6 py-4 text-sm font-bold"><?= htmlspecialchars($file['file_name']) ?></td>
                                            <td class="px-6 py-4 text-sm text-slate-600 truncate max-w-xs" title="<?= htmlspecialchars($file['file_description'] ?? '') ?>"><?= htmlspecialchars($file['file_description'] ?? '-') ?></td>
                                            <td class="px-6 py-4 text-sm">
                                                <div class="flex gap-2">
                                                    <a href="<?= htmlspecialchars($file['file_url']) ?>" target="_blank" class="p-2 text-primary hover:bg-primary/10 rounded-lg transition-colors" title="Buka File"><span class="material-symbols-outlined">open_in_new</span></a>
                                                    <button onclick="openEditModal(<?= htmlspecialchars(json_encode($file)) ?>)" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit"><span class="material-symbols-outlined">edit</span></button>
                                                    <button onclick="confirmDelete(<?= $file['id'] ?>)" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus"><span class="material-symbols-outlined">delete</span></button>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                    ...
                    <?php if ($totalPages > 1): ?>
                        <div class="p-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                            ...
                            <div class="flex gap-1">
                                <?php if ($page > 1): ?>
                                    <a href="?page=<?= $page - 1 ?>" class="px-3 py-1 border border-slate-200 dark:border-slate-700 rounded text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Sebelumnya</a>
                                <?php endif; ?>
                                <?php if ($page < $totalPages): ?>
                                    <a href="?page=<?= $page + 1 ?>" class="px-3 py-1 border border-slate-200 dark:border-slate-700 rounded text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Selanjutnya</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endif; ?>
    ...
    <div id="modal-edit" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form action="/admin/pengabdian/dokumen-mandiri/edit" method="POST" class="p-6 space-y-4">
                <input type="hidden" name="id" id="edit-id">
                <div class="space-y-2">
                    <label class="text-sm font-semibold">Nama Dokumen</label>
    ...
    <div id="modal-delete" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden p-8 text-center animate-in fade-in zoom-in duration-200">
            <div class="scale-110 size-20 bg-red-100 dark:bg-red-900/30 text-red-600 rounded-full flex items-center justify-center mx-auto mb-6">
                <span class="material-symbols-outlined text-4xl">delete_forever</span>
            </div>
            <h3 class="text-xl font-bold mb-2">Hapus Dokumen?</h3>
            <p class="text-slate-500 dark:text-slate-400 text-sm mb-8">Tindakan ini tidak dapat dibatalkan. Data akan dihapus secara permanen.</p>
            <form action="/admin/pengabdian/dokumen-mandiri/hapus" method="POST" class="flex flex-col gap-3">
    ...
    <script>
        ...
        window.onclick = function(event) {
            if (event.target.classList.contains('bg-black/50')) {
                const modals = ['modal-edit', 'modal-delete'];
