...
<head>
    ...
    <title>Kelola Buku Panduan KKN Tematik</title>
</head>
...
<body class="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display">
        ...
        <main class="flex-1 flex flex-col overflow-hidden pt-14 lg:pt-0">
            <div class="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
                <header class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 class="text-xl md:text-3xl font-black tracking-tight mb-2">Kelola Buku Panduan KKN Tematik</h2>
                        <p class="text-slate-500">Tambah, edit, atau hapus buku panduan KKN tematik dalam sistem.</p>
                    </div>
                    <a href="/buku-panduan-kkn-tematik" target="_blank" class="inline-flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-6 py-3 rounded-xl text-primary font-bold hover:bg-primary hover:text-white transition-all shadow-sm active:scale-95 text-sm w-fit">
                ...
                <section class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 mb-8 shadow-sm">
                    <div class="flex items-center gap-2 mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
                        ...
                        <h3 class="text-lg font-bold">Formulir Input Buku Panduan</h3>
                    </div>
                    <form action="/admin/kkn/panduan-tematik/tambah" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <section class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                        <h3 class="text-lg font-bold">Daftar File Terunggah</h3>
                        <div class="relative">
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-slate-800/50">
                                    ...
                                    <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Link GDrive</th>
                                ...
                                <?php if (empty($files)): ?>
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-slate-500">
                                            ...
                                            <p>Belum ada buku panduan yang ditemukan.</p>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($files as $index => $file): ?>
                                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                            <td class="px-6 py-4 text-sm font-medium text-slate-400"><?= ($page - 1) * $limit + $index + 1; ?></td>
                                            <td class="px-6 py-4 text-sm font-bold text-slate-900 dark:text-white"><?= htmlspecialchars($file['file_name']); ?></td>
                                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400 max-w-xs truncate" title="<?= htmlspecialchars($file['file_description'] ?? ''); ?>"><?= htmlspecialchars($file['file_description'] ?? '-'); ?></td>
                                            <td class="px-6 py-4 text-sm">
                                                <a class="text-primary hover:text-orange-600 flex items-center gap-1 font-semibold group" href="<?= htmlspecialchars($file['file_url']); ?>" target="_blank">
                                                    <span class="material-symbols-outlined text-base group-hover:scale-110 transition-transform">open_in_new</span>
                                                    Buka Link
                                                </a>
                                            </td>
                    ...
                    <div class="p-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <p class="text-sm text-slate-500">Menampilkan <?= count($files); ?> dari <?= $totalItems; ?> file</p>
                        <?php if ($totalPages > 1): ?>
                            <div class="flex gap-1">
                                ...
                                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                    <a href="?page=<?= $i; ?>" class="px-3 py-1 border <?= $i === $page ? 'bg-primary text-white border-primary font-bold' : 'border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800' ?> rounded text-sm transition-colors"><?= $i; ?></a>
                                <?php endfor; ?>
    ...
    <div id="modal-edit" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <h3 class="text-lg font-bold">Edit Buku Panduan</h3>
            ...
            <form action="/admin/kkn/panduan-tematik/edit" method="POST" class="p-6 space-y-4">
    ...
    <div id="modal-delete" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden text-center p-8">
            ...
            <form action="/admin/kkn/panduan-tematik/hapus" method="POST" class="flex flex-col gap-3">
