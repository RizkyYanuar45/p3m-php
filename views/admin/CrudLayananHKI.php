...
<head>
    ...
    <title>Kelola Layanan HKI</title>
</head>
...
<body class="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display">
        ...
        <main class="flex-1 flex flex-col overflow-hidden pt-14 lg:pt-0">
            <div class="flex-1 overflow-y-auto p-4 md:p-8 bg-background-light dark:bg-background-dark custom-scrollbar">
                <header class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 class="text-xl md:text-3xl font-black tracking-tight mb-2">Kelola Layanan HKI</h2>
                        <p class="text-slate-500">Manajemen berkas panduan pengelolaan jurnal ilmiah dalam sistem.</p>
                    </div>
                    <a href="/panduan-layanan-hki" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg font-bold text-sm transition-all border border-slate-200 dark:border-slate-700">
                ...
                <section class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 mb-8 shadow-sm">
                    <div class="flex items-center gap-2 mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
                        ...
                        <h3 class="text-lg font-bold">Tambah Layanan HKI Baru</h3>
                    </div>
                    <form action="/admin/layanan/hki/tambah" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="flex flex-col gap-2">
                            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Nama File</label>
                            <input name="file_name" required class="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:border-primary focus:ring-primary h-12" placeholder="Contoh: Laporan Akhir 2023" type="text" />
                        </div>
                        <div class="flex flex-col gap-2">
                        ...
                        <div class="md:col-span-2 flex justify-end">
                            <button class="bg-primary hover:bg-primary/90 text-white font-bold py-3 px-8 rounded-lg transition-all flex items-center gap-2" type="submit">
                                <span class="material-symbols-outlined text-xl">save</span> Simpan File
                            </button>
                        </div>
                    </form>
                </section>
                ...
                <section class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                        <h3 class="text-lg font-bold">Daftar File</h3>
                        <div class="relative">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800" id="file-table-body">
                                <?php if (empty($files)): ?>
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center text-slate-500">Belum ada file yang ditemukan.</td>
                                    </tr>
                                <?php else: ?>
    ...
    <div id="modal-edit" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <h3 class="text-lg font-bold">Edit Layanan HKI</h3>
            ...
            <form action="/admin/layanan/hki/edit" method="POST" class="p-6 space-y-4">
                ...
                <div class="space-y-2">
                    <label class="text-sm font-semibold">Link GDrive</label>
    ...
    <div id="modal-delete" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden p-8 text-center animate-in fade-in zoom-in duration-200">
            ...
            <h3 class="text-xl font-bold mb-2">Hapus Layanan HKI?</h3>
            <p class="text-slate-500 dark:text-slate-400 text-sm mb-8">Tindakan ini tidak dapat dibatalkan. File akan dihapus secara permanen.</p>
            <form action="/admin/layanan/hki/hapus" method="POST" class="flex flex-col gap-3">
